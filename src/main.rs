...
fn main() {
    ...
    // Download the selected version's files
    ...
    let version_json_str = std::fs::read_to_string(&version_json_path).expect("Failed to read version JSON");
    let version_detail: utils::VersionDetail = serde_json::from_str(&version_json_str).expect("Failed to parse version JSON");
    ...
    let asset_index_json: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(format!("minecraft/assets/indexes/{}.json", version_detail.assetIndex.id)).expect("Failed to read asset index JSON")
    ...
    let args = vec![
        ...
        "--assetIndex", &version_detail.assetIndex.id,
        "--accessToken", "0",
        "--uuid", "0",
        "--userType", "msa",
        "--clientId", "0",
